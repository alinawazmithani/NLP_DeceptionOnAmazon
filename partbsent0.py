...
def loadData(path, Text=None):
    with open(path, encoding='utf-8') as f:
        ...
        next(reader)
        for line in reader:
            (Id, Text, Rating) = parseReview(line)
            rawData.append((Id, Text, Rating))
...
def splitData(percentage):
    ...
    for (_, Text, Rating) in rawData[:trainingSamples] + rawData[halfOfData:halfOfData+trainingSamples]:
        trainData.append((toFeatureVector(preProcess(Text,Rating)),Rating))
    for (_, Text, Rating) in rawData[trainingSamples:halfOfData] + rawData[halfOfData+trainingSamples:]:
        testData.append((toFeatureVector(preProcess(Text,Rating)),Rating))
...
def parseReview(reviewLine):
    ...
    return (int(reviewLine[0]), reviewLine[8], 'positive' if int(reviewLine[2]) > 3 else 'negative' if int(reviewLine[2]) < 3 else 'neutral')
...
def preProcess(text,rating):
    ...
    text_final = ''
    if rating == 'negative' or rating == 'positive':
        text_final = text
    text = ''
    text = nltk.word_tokenize(text_final)
